...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("sync_example_single_client_server", |b| {
        b.iter(|| sync_example_single_client_server(&mut client_managers[0], &mut server_manager))
    });
    ...
    c.bench_function("sync_example_multiclient_server 4", |b| {
        b.iter(|| sync_example_multiclient_server(&mut client_managers, &mut server_manager))
    });
    drop(client_managers);
    drop(server_manager);
...
criterion_group! {
    name = benches;
    config = Criterion::default().measurement_time(std::time::Duration::from_secs(60)).sample_size(100000);
